...
setInterval(function(){
    let date = new Date();
    today.innerHTML = `${date.getDay() == 0 ? "Chủ Nhật" : "Thứ "+ (date.getDay()+1)} <span class ="text-danger">${date.getHours() < 10 ? "0" + date.getHours():date.getHours()}:${date.getMinutes() < 10 ? "0" + date.getMinutes():date.getMinutes()}:${date.getSeconds() < 10 ? "0" + date.getSeconds():date.getSeconds()}</span> Time không dừng lại => đừng dại ngồi chơi :))`;
}, 1000);
